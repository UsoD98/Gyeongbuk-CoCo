...
> 짝 문서: [`FE_개발_진행상황.md`](./FE_개발_진행상황.md) (v1.1 백로그) · [`FE_계약_추적표.md`](./FE_계약_추적표.md) (계약)
> ℹ️ v1.0 정리(2026-09-20)로 `FE_API_현황.md`·`FE_API_연동가이드.md` 는 삭제됐다. 이 문서에 남은 두 문서 참조는 **구현 당시 기록**이며, 원문이 필요하면 `git show v1.0:docs/<파일>`.
> ⚠️ **메인 스파인(Step 0~7)·섬 M·섬 P 는 v1.0 으로 전량 완료됐다.** 지금 유효한 부분은 §그룹 F·§그룹 R 의 잔여 5건 사양뿐이다.
...
- **가이드**: 구현 상세가 있던 `FE_API_연동가이드.md` 섹션 (v1.0 정리로 삭제 — `git show v1.0:docs/FE_API_연동가이드.md`)
