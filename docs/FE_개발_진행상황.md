...
> 최종 업데이트: 2026-08-08 (S4 · GBC012 코스 상세 완료 + 브라우저 E2E 검증 — 카드→상세·새로고침 재진입·`placeName` 실명 렌더 확인, 401은 인터셉터 로그인 리다이렉트로 처리)
...
| 메인 스파인 (코스) | 4 / 8 | S1·S2·S3·S4 완료. 다음 S5(삭제). Step 8은 ⏸ |
...
- [ ] `☐` **S5 · GBC013 코스 삭제** ▶ — 의존: S3(/S4). `deleteCourse`+확인 다이얼로그. DoD: 삭제 후 목록에서 사라짐. → 순서 §Step5  ← **다음 착수 후보**
