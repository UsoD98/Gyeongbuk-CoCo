...
> 최종 업데이트: 2026-08-08 (S6 · GBC015 코스 제목 수정 완료 — 요약 헤더 인라인 편집(`EditableCourseTitle`)→`PATCH /tour-course/{id}/title` 낙관적 업데이트+실패 롤백, `plannerStore.setTitle`·`useCourseTitle` 훅 신설. lint·build 통과. 실백엔드 E2E는 dev+백엔드+로그인 시 확인)
...
| 메인 스파인 (코스) | 6 / 8 | S1·S2·S3·S4·S5·S6 완료. 다음 S7(공유). Step 8은 ⏸ |
...
- [ ] `☐` **S7 · GBC014 공유 + 공개뷰** ▶ — 의존: S2, S4. `shareRouter`(가드 밖)+`getPublicCourse`+카카오 공유. DoD: 비로그인 링크로 코스 표시. → 순서 §Step7  ← **다음 착수 후보**
