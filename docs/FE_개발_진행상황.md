...
> 최종 업데이트: 2026-08-08 (P0 · POI 목→API 교체 준비 완료 — `hooks/usePoiList`(목록, P2 교체점)·`hooks/usePoi`(단일 상세, P3 교체점) 신설 + `ResultsPanel`이 `@/mocks` 직접 import 대신 `usePoiList` 경유·`PoiDrawer`가 `resolvePoi` 직접 호출 대신 `usePoi` 경유 + `api/poi.ts`에 역방향 매핑 `CAT_BY_CONTENT_TYPE` 추가. 순수 리팩터(로직·목 데이터 동일) → lint·build 통과. P2/P3에서 훅 내부만 API로 교체하면 소비처 무수정)
...
| 🏝️ 섬 P · POI | 1 / 4 | P0 완료. P1 착수 가능. P2·P3은 ⏸ |
