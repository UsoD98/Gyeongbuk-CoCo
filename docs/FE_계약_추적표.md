...
> 짝 문서: [`FE_개발순서.md`](./FE_개발순서.md) §Step0 0-A · [`openapi.yaml`](./openapi.yaml) (스펙)
> **백엔드에 넘길 요청서: [`BE_계약_요청서.md`](./BE_계약_요청서.md)** — 3건(#4→R1 · #8→R2 · #9→R3)을 근거·요청안·회신 양식으로 정리했다. **R2 는 회신·반영 완료(0.6.3), R3 도 회신·반영 완료(0.6.4~0.6.7 → FE F9), R1 은 대기.**
> 목적: 연동 착수 전 확정되지 않았던 계약을 추적한다.
> **연동 중 400/422/빈결과가 나면 이 표부터 확인한다.**
> 최종 업데이트: 2026-09-20 (**생성 응답(GBC010)에 `login` 추가 예정 → FE 선반영**(#11) — 로그인 상태로 생성하면 서버가 코스를 요청자 계정에 **바로 귀속**하므로, FE 는 `login:true` 일 때 게스트용 인메모리 경로(`/planner/`) 대신 **`/planner/{courseId}`** 로 보내 소유 코스 화면(제목 인라인 편집 GBC015 · 변경 저장 GBC020)에서 그대로 이어 작업하게 한다. 필드가 아직 안 오면 `undefined` → 게스트와 동일 동작이라 **백엔드 배포 순서와 무관하게 안전**하다.)
> 이전 업데이트: 2026-08-29 (**생성 응답(GBC010)에 `title` 추가 → FE 반영** — 백엔드 `TourCourseGenerateResponseDto` 에 `title` 이 있고 `buildGenerateResponse(courseId, savedCourse.getTitle(), …)` 로 **DB 에 저장된 제목 그대로** 내려온다(소스 실측). 제목은 `buildDefaultTitle(sigunguCodes)` 가 짓는다 — 시군구 1개면 '경주 여행 코스', 복수면 '경주 외 2곳 여행 코스', 미선택·조회 실패면 '여행 코스'(**항상 non-null**). FE 는 홈에서 자체 생성하던 제목(`'{목적지} 여행 코스'`/`'AI 추천 코스'`)을 **폴백으로 격하**하고 `res.title` 을 정본으로 쓴다 → 생성 직후 플래너 제목이 목록(GBC011)·상세(GBC012)에서 보이는 제목과 **같아진다**(전에는 어긋났다).)
> 이전 업데이트: 2026-08-29 (**R3 회신 수용 → #9 `☑ 확정(소스 실측)`** — 백엔드가 요청안 ㉯ 를 택해 조회 응답에 찜 정보를 실었다. 소스 실측(`../back` a856895/0.6.8 기준): **0.6.4** 가 `PoiCurationItemDto`(목록)·`PoiDetailResponseDto`(상세)에 `liked`(boolean)를 넣고 **상세에만** `totalLiked`(int, `poi_rating.likes`)를 추가, **0.6.5** 가 토글 응답 `PoiLikeResponseDto` 의 `likes` 를 **`totalLiked` 로 개명**(조회와 필드명 통일 → 재조회 없이 patch 가능), **0.6.7** 이 목록·상세에 `stars`(BigDecimal `DECIMAL(3,1)`, nullable)를 추가했다. 두 조회 API 는 `permitAll` 이라 비로그인·탈퇴 사용자면 예외 없이 `liked:false`, 평점 행이 없으면 `stars:null`·`totalLiked:0` 이다(에러 아님). ⚠️ **목록에는 `totalLiked` 가 없다**(설계상 상세 전용). FE 는 F9 에서 전량 반영했고, 그 과정에서 **개명(0.6.5)을 따라가지 못해 좋아요 수가 유실되던 실결함**을 함께 고쳤다. **로그인 E2E 까지 실측 완료**(사용자가 로그인 대행): 토글 200 `{liked,totalLiked}` → 카드·드로어 즉시 반영 · **새로고침 후 하트 유지**(이 표 #9 가 겨냥한 문제 해소) · 드로어 재오픈(캐시)에도 토글 결과 보존 · 세션 전환 시 제거/복구 · 콘솔 에러 0.)
> 이전 업데이트: 2026-08-22 (**R2 회신 수용 → #8 `☑ 확정(라이브)`** — 백엔드 `cd6c7ec`(0.6.3)가 코스 상세·공개뷰 응답에 `mapx`/`mapy` 를 추가했다. 로컬 백엔드를 실제로 띄워 `GET /tour-course/1/view` 로 실측: 좌표가 **JSON number**(`129.2423293844`)로 오고, POI 캐시에 없는 `contentId` 는 `null`(그 장소는 `placeName` 도 `''`) — 코스 1의 18곳 중 6곳이 그렇다. FE 는 `CoursePlace.mapx/mapy` 를 받아 `Poi.lat/lng` 로 주입하고 장소명 지오코딩을 **좌표 null 전용 폴백으로 격하**했다(제거하지 않음 — 캐시 미스가 실재한다). Vite dev 모듈을 직접 태운 실측: 18곳 중 **12곳 즉시 렌더 · 지오코딩 왕복 0회**. ⚠️ **생성(GBC010)은 다른 DTO 라 여전히 좌표 없음**(그 화면은 GBC017 좌표로 그림 → 추가 요청 안 함). #9 는 "추가 예정" 구두 회신을 받아 `🕒 예정` 로 갱신.)
...
### 10. 소셜 계정 비밀번호 변경 — ☑ FE 에서 막지 않기로 결정 (2026-09-06)
- **서버가 이미 정확히 막는다**: `UserServiceImpl.updatePassword` 는 `user.getPassword() == null` 이면 **"소셜 로그인 유저는 비밀번호를 변경할 수 없습니다."** 로 거절한다. FE 는 `getApiErrorMessage` 로 그 메시지를 그대로 toast 에 띄운다.
- **FE 가 대신 판단할 근거가 없다**: GBC006(`UserInfoResponseDto`)에 `provider` 도 `hasPassword` 도 없다.
- **`provider` 는 대용이 될 수 없다**: `KakaoOAuthService.registerKakaoUser` 가 같은 이메일의 **기존 로컬 계정에 카카오를 연결**(`User.linkKakao`)하면 `provider='kakao'` 가 되지만 **비밀번호는 남는다**. 로그인 수단으로 가리면 그 계정은 비밀번호가 있는데도 변경 UI 를 잃는다(오동작은 아니고 기능 손실).
- **결정**: 로그인 수단으로 미리 가리는 처리를 넣었다가 **되돌렸다**(`defb72a` → revert). 부정확한 선차단보다 서버 판정을 그대로 보여 주는 쪽이 낫다.
- **다시 열 조건**: 백엔드가 GBC006 에 **`hasPassword: boolean`**(= `password != null`)을 실어 주면 그 값으로 폼을 감출 수 있다. 값이 아니라 보유 여부라 노출 위험이 없다. 그때까지는 서버 에러 메시지로 안내한다.
...
- ✅ **생성 응답(GBC010)에 `title` 과 `contentName` 이 실린다(소스 실측 2026-08-29)** — `TourCourseGenerateResponseDto` = `{courseId, title, schedule[{date, places[{seq, time, type, contentId, contentName, durationMinutes, thumbnailImg, operatingHours, cost}]}]}`.
  - `title`: 저장된 코스 제목을 그대로 반환(`buildGenerateResponse(courseId, savedCourse.getTitle(), …)`). `buildDefaultTitle(sigunguCodes)` 규칙 — 1개='경주 여행 코스' · 복수='경주 외 2곳 여행 코스' · 미선택/이름 조회 실패='여행 코스'. **항상 non-null** → FE 는 이 값을 제목의 정본으로 쓰고(`plannerStore.loadFromApi`), 홈에서 만든 문자열은 빈 값일 때만 쓰는 폴백이다.
  - `contentName`: 위 69행의 "추가 예정"이 **반영 완료**(`titleOf(summaryMap, contentId)`) — POI 캐시 미스면 `''`. FE 의 `placeName || contentName || '장소 #id'` 폴백은 그대로 유효하다.
  - ⚠️ 헤더(`peopleCount`·기간·`transport`·`theme`)와 좌표(`mapx`/`mapy`)는 여전히 없다(상세/공개뷰 DTO 전용).
...
| 9 | 찜 상태 조회 API (F4) | 로그인 사용자의 찜 상태를 **읽을** 경로를 주실 수 있는가? (선택지: ㉮ `GET /poi/likes` → 내가 찜한 `contentId[]`, ㉯ `GET /poi`·`GET /poi/{contentId}` 응답에 `liked`(+`likes`) 필드 추가 — 인증 있을 때만 채움) | **☑ 해결 — 백엔드가 ㉯ 를 택해 조회 응답에 실어 줬다(소스 실측 2026-08-29, `a856895`/0.6.8).** **0.6.4**: 목록 `PoiCurationItemDto` += `liked`, 상세 `PoiDetailResponseDto` += `liked`·`totalLiked`(int, `poi_rating.likes`). **0.6.5**: 토글 `PoiLikeResponseDto` 의 `likes` → **`totalLiked`** 개명(조회와 필드명 통일 — 재조회 없이 patch 하라는 백엔드 메모). **0.6.7**: 목록·상세 += `stars`(BigDecimal `DECIMAL(3,1)`, nullable). ⚠️ **목록에는 `totalLiked` 가 없다**(상세 전용). 두 조회는 `permitAll` — 비로그인·탈퇴면 `liked:false`, 평점 행 없으면 `stars:null`·`totalLiked:0`. | **FE 반영 완료(F9, 2026-08-29)** — `poiLikeStore` 에 `hydrate()` 추가(목록·상세 응답의 `liked`/`totalLiked` 주입, **이미 아는 값은 덮지 않아** 방금 토글한 결과를 낡은 응답이 되돌리지 못한다), `usePoiList`·`usePoi` 가 응답을 hydrate 하고 **세션 키를 조회·캐시 키에 포함**(`liked` 가 사용자별 값이라 로그인 전후 응답 공유·재사용 금지 → 로그인하면 자동 재조회), `usePoiLike` 는 토글 응답 `{liked,totalLiked}` 를 그대로 확정. 개명(0.6.5)을 따라가지 못해 **좋아요 수가 유실되던 실결함**도 함께 수정(`res.likes` → `res.totalLiked`). `stars` 는 `Poi.rating` 으로 옮기고 0(평점 없음)이면 `★` 를 그리지 않는다. 메모리 전용이라는 성격은 그대로지만, 이제 조회가 `liked` 를 주므로 새로고침 후 목록·상세를 부르면 서버 상태로 복구된다(**로그인 E2E 로 실측 확인**) | ☑ 확정(라이브) |

| # | 항목 | 질문 | FE 현재 처리 | 상태 |
|---|------|------|--------------|:---:|
| 11 | 생성 응답 `login` (GBC010) | 생성 요청이 **로그인 상태로 처리됐는지**(= 서버가 코스를 요청자 계정에 귀속했는지)를 응답에 실어 줄 수 있는가? | **🕒 백엔드 추가 예정**(사용자 전달, 2026-09-20) — `TourCourseGenerateResponseDto` 에 `login: boolean` 추가. FE 는 **선반영**했다: `CreateCourseResponse.login?: boolean`(옵셔널 → 필드 미도착이면 `undefined`, 게스트와 동일 취급). `false`/미도착이면 기존대로 `/planner/`(인메모리 index 라우트)로 가고 저장 버튼은 소유권 이전(GBC016 assign)을 태운다. `true` 면 `navigate('/planner/{courseId}', { state: { fromCreate: true } })` — 목록 카드 클릭과 **같은 경로**라 `Planner` 의 소유 판정(`owned`)이 즉시 켜져 제목 인라인 편집(GBC015)·변경 저장(GBC020)이 열린다. ⚠️ 이때 상세(GBC012) 재조회는 **건너뛴다**(`fromCreate`) — `loadDetail` 이 스토어를 통째로 갈아끼워 홈에서 고른 지역(`search.dests`)과 그 사이 편집분을 날리기 때문. 새로고침하면 스토어가 비어 정상적으로 상세를 불러온다 | 🕒 예정 · FE 선반영 |
