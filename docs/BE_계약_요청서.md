...
| **R2** | 코스 장소에 좌표(`mapx`/`mapy`) | 컬렉션에서 내 코스를 열면 **지도에 마커가 0개**. 항목 간 이동시간 추정도 표기되지 않음 | 응답 `schedule[].places[]` 에 `mapx`/`mapy` 추가 | DTO 2개에 2필드 + 헬퍼 1개 (**추가 조회 없음**) | 지오코딩 폴백 제거, 마커·이동시간 정확도 상승 |
| **R3** | 내 찜(POI 좋아요) 상태 **조회** 경로 | 새로고침하면 찜한 곳의 **하트가 비어 보이고**, 그 하트를 누르면 추가가 아니라 **해제**된다 | ㉯ 목록·상세 응답에 `liked` 필드(인증 시에만 채움) | 조회 1건 + DTO 필드 | 초기 상태 주입만 추가(구조 변경 없음) |

우선순위: **R3 ≥ R1 > R2**. R3은 이미 저장된 데이터가 사용자에게 잘못 보이는 **정합성 결함**이고, R1은 기능 4개를 통째로 막는 **블로커**, R2는 기능은 도는데 폴백 품질 문제다.
...
## R2. 코스 장소에 좌표(`mapx`/`mapy`) 추가 (추적표 #8)
