...
>
> ⚠️ **이 문서는 2026-07-17 시점의 스냅샷이며 이후 갱신하지 않는다.** 현재 진행 상황의 정본은 [`FE_개발_진행상황.md`](./FE_개발_진행상황.md) 다. 아래 '🔴 missing' 판정 상당수는 이미 해소됐다 — 특히 **GBC006~009(마이페이지)는 2026-09-06 섬 M 에서 전량 구현**됐고(`/mypage`), 5장의 `userId` 미보관 지적도 accessToken 클레임 디코드(`utils/jwt.ts`)로 해결됐다(추적표 #4).
