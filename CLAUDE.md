...
실 백엔드(`../back`, :8080)는 **Java 런타임과 DB 접속정보(`back/.env`)가 있어야 뜬다.** 이 개발
환경에는 Java 가 설치돼 있지 않으므로(`java`·`JAVA_HOME` 부재) API 왕복이 필요한 검증은 백엔드를
띄울 수 있는 환경에서 하거나 사용자에게 대행을 요청한다.
