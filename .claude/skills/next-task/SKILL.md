---
name: next-task
description: 경북 CoCo 프론트엔드 개발을 v1.1 백로그 기준으로 한 단계 이어서 진행한다. docs/FE_개발_진행상황.md를 읽어 다음 착수할 Task를 파악하고, 사용자에게 진행 여부를 물은 뒤, 상세 사양대로 구현·검증·보드 갱신·커밋까지 수행한다. "다음 진행", "이어서 개발", "다음 할 일 해줘", "/next-task"를 요청할 때 사용.
---
...
## 1. 진행상황 파악
- **정본**: `docs/FE_개발_진행상황.md`(v1.1 백로그). 항상 먼저 읽는다. 이 보드는 **활성 Task(`☐`·`◐`·`⏸`)의 상세 + v1.0 완료 Task 한 줄 요약표**만 담는다.
  - 📦 `docs/FE_개발_진행상황_아카이브.md`(완료 Task 상세·과거 진행 로그·헤더 이력)는 **읽지 않는다** — 다음 Task 판단에 불필요하고 크다(≈230KB). 과거 구현 근거를 반드시 되짚어야 할 때만 해당 섹션을 지목해 연다.
  - 📐 `docs/DECISIONS.md`(현행 구현 규약 + 뒤집지 말 설계 결정)는 **구현 착수 전 반드시 읽는다** — 짧고, 새 UI·오버레이·지도 요소를 넣을 때 걸리는 규칙이 전부 여기 있다.
- **다음 착수 후보** = 의존(선행 Task)이 모두 `☑`이고, 상태가 `☐`(대기)이며, `⏸`(백엔드 대기)가 **아닌** Task 중 우선순위 최상위.
  ...
  - ⚠️ **v1.0 배포 완료(태그 `v1.0`, 2026-09-20) 이후 보드에 `☐` 는 0개다.** 남은 5건(F1·F2·F6·F8 ㉡·R8)은 전부 `◐` 이고 각각 바깥 조건을 기다린다 — **로그인 대행**(F1·F6) · **백엔드 필드**(F2) · **사용자 설계 판단**(F8 ㉡·R8 마커, 근거는 `DECISIONS.md` §2-2·§2-3). 즉 **사용자 승인 없이 자동으로 집어들 Task 가 없다** → 2단계에서 어느 조건을 풀 수 있는지부터 묻는다.
  - 새 요구가 들어오면 보드에 `☐` Task 로 추가하고 사양은 `docs/FE_개발순서.md` 를 따르거나 새로 적는다. `FE_개발순서.md` 는 **§그룹 F·§그룹 R 만 유효**하고 나머지(메인 스파인·섬 M·섬 P)는 v1.0 으로 완료됐다.
  - 심각도 배지 `🔴/🟠/🟡` 는 **Task ID 가 아니다**(섬 P 의 P0·P1·P2 와 혼동 금지).
  - 의존 사슬은 `docs/FE_개발순서.md`의 "핵심 의존 사슬"과 각 Step의 **의존** 항목으로 확인한다.
- `⏸` Task(스펙 `보류`/`개발중`)는 백엔드 완료 전까지 착수하지 않는다.
...
- 대상 Task 사양을 읽는다:
  ...
  - `docs/FE_계약_추적표.md` 해당 항목 (계약 확정 여부 · 400/빈결과 원인)
  - `docs/openapi.yaml` 해당 엔드포인트의 200 응답 예시
  - `docs/DECISIONS.md` — 건드리는 영역의 규약(§1)과 뒤집으면 안 되는 결정(§2)
